...
def manual_select_keypoints(images, titles=["Reference Image", "Moving Image"]):
    ...
    def redraw_images():
        for img_index in range(2):
            img = original_images[img_index].copy()
            for i, (x, y) in enumerate(keypoints[img_index]):
                cv2.circle(img, (x, y), 5, (0, 255, 0), -1)
                cv2.putText(img, str(i + 1), (x, y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
            images[img_index] = img
            cv2.imshow(titles[img_index], img)
    ...
    original_images = [img.copy() for img in images]  # Store the original images
